 ...
 * TransactionRepository, using the DB-driven Rule row's timeline (minutes) as
...
public class VelocityAnomalyRule implements RiskRule {
    ...
    public RuleResult evaluate(Transaction transaction, HistoricalProfile profile, Rule rule) {
        ...
        LocalDateTime from = now.minusMinutes(Math.max(1, rule.getTimeline()));
        ...
        String reason = String.format("%d transactions detected within the last %d minute(s) (threshold=%d)",
